...
from grading.models import Assignment, Tenant, Course, Class, Semester, UserProfile
...
class DeleteAssignmentTestCase(TestCase):
    ...
    def test_delete_by_non_owner_raises_permission_error(self):
        ...
        # 验证作业未被删除
        self.assertTrue(
            Assignment.objects.filter(id=self.assignment.id).exists()
        )
    ...
    def test_delete_workflow(self):
        """测试完整的删除工作流程"""
        # 第一步：获取影响信息
        result1 = self.service.delete_assignment(
            self.assignment,
            self.teacher,
            confirm=False
        )
        ...
        # 第二步：确认删除
        result2 = self.service.delete_assignment(
            self.assignment,
            self.teacher,
            confirm=True
        )
