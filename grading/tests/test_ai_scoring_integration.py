...
class AIScoreViewTest(TestCase):
    ...
    def test_ai_score_success(self):
        ...
        with patch(
            "grading.views.volcengine_score_homework", return_value=(mock_score, mock_comment)
        ):
            response = self.client.post(
                reverse("grading:ai_score"),
                {"file_path": "test_homework.docx", "repo_id": self.repo.id},
            )
    ...
    def test_ai_score_locked_file(self):
        ...
        response = self.client.post(
            reverse("grading:ai_score"),
            {"file_path": "locked_homework.docx", "repo_id": self.repo.id},
        )
    ...
    def test_ai_score_missing_file_path(self):
        """测试缺少文件路径参数"""
        response = self.client.post(reverse("grading:ai_score"), {"repo_id": self.repo.id})
    ...
    def test_ai_score_invalid_file_path(self):
        """测试无效的文件路径"""
        response = self.client.post(
            reverse("grading:ai_score"), {"file_path": "nonexistent.docx", "repo_id": self.repo.id}
        )
    ...
    def test_ai_score_grade_conversion(self):
        ...
        for score, expected_grade in test_cases:
            with self.subTest(score=score):
                with patch(
                    "grading.views.volcengine_score_homework", return_value=(score, "测试评价")
                ):
                    response = self.client.post(
                        reverse("grading:ai_score"),
                        {"file_path": "test_homework.docx", "repo_id": self.repo.id},
...
class BatchAIScoreViewTest(TestCase):
    ...
    def test_batch_ai_score_success(self):
        ...
        with patch("grading.views.volcengine_score_homework", side_effect=mock_ai_score):
            response = self.client.post(
                reverse("grading:batch_ai_score"),
                {"dir_path": "homework_batch", "repo_id": self.repo.id},
            )
    ...
    def test_batch_ai_score_empty_directory(self):
        ...
        response = self.client.post(
            reverse("grading:batch_ai_score"), {"dir_path": "empty_dir", "repo_id": self.repo.id}
        )
    ...
    def test_batch_ai_score_missing_dir_path(self):
        """测试缺少目录路径参数"""
        response = self.client.post(reverse("grading:batch_ai_score"), {"repo_id": self.repo.id})
    ...
    def test_batch_ai_score_invalid_directory(self):
        ...
        response = self.client.post(
            reverse("grading:batch_ai_score"),
            {"dir_path": "nonexistent_dir", "repo_id": self.repo.id},
        )
...
class AIScoreErrorHandlingTest(TestCase):
    ...
    def test_ai_score_empty_file(self):
        ...
        with patch("grading.views.volcengine_score_homework", return_value=(None, "内容为空")):
            response = self.client.post(
                reverse("grading:ai_score"), {"file_path": "empty.docx", "repo_id": self.repo.id}
            )
    ...
    def test_ai_score_exception_handling(self):
        ...
        with patch("grading.views.volcengine_score_homework", side_effect=Exception("网络错误")):
            ...
            response = self.client.post(
                reverse("grading:ai_score"), {"file_path": "test.docx", "repo_id": self.repo.id}
            )
    ...
    def test_ai_score_permission_denied(self):
        ...
        other_user = User.objects.create_user(
            username="otheruser", password="testpass123", is_staff=True
        )
        other_repo = Repository.objects.create(
            name="其他仓库", owner=other_user, repo_type="local", path=self.temp_dir, is_active=True
        )

        response = self.client.post(
            reverse("grading:ai_score"), {"file_path": "test.docx", "repo_id": other_repo.id}
        )
