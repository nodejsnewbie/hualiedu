...
from grading.models import CommentTemplate, Tenant, UserProfile
...
class CommentIntegrationTestCase(TestCase):
    ...
    def setUp(self):
        ...
        self.teacher = User.objects.create_user(
            username="teacher1", password="password123", email="teacher1@test.edu.cn"
        )
    ...
    def test_record_new_comment_usage(self):
        ...
        self.assertFalse(
            CommentTemplate.objects.filter(
                tenant=self.tenant, teacher=self.teacher, comment_text=new_comment
            ).exists()
        )
    ...
    def test_system_templates_fill_gap(self):
        """测试系统模板补充到5个"""
        # 删除一些个人模板，只保留2个
        CommentTemplate.objects.filter(tenant=self.tenant, teacher=self.teacher).exclude(
            id=self.personal_template1.id
        ).delete()
    ...
    def test_comment_deduplication(self):
        ...
        templates = CommentTemplate.objects.filter(
            tenant=self.tenant, teacher=self.teacher, comment_text=comment_text
        )
