...
class EndToEndGradingWorkflowTest(TestCase):
    ...
    def create_normal_homework(self, filename="李四.docx"):
        ...
        doc.add_paragraph(
            "答：OSI七层模型包括物理层、数据链路层、网络层、传输层、会话层、表示层和应用层..."
        )
...
class EndToEndWebInterfaceTest(TestCase):
    ...
    def setUp(self):
        ...
        self.teacher = User.objects.create_user(
            username="webteacher", password="testpass123", is_staff=True
        )
        ...
        # 创建学生用户
        self.student = User.objects.create_user(username="webstudent", password="testpass123")

        self.student_profile = UserProfile.objects.create(
            user=self.student, tenant=self.tenant, is_tenant_admin=False
        )
    ...
    def test_teacher_creates_course_and_class(self):
        ...
        class_obj = class_service.create_class(course=course, name="计算机1班", student_count=30)
    ...
    def test_student_uploads_homework(self):
        ...
        class_obj = class_service.create_class(course=course, name="测试班级", student_count=20)
        ...
        repository = repo_service.create_filesystem_repository(
            teacher=self.teacher, class_obj=class_obj, name="学生作业仓库"
        )
        ...
        homework = Homework.objects.create(
            course=course, title="第一次作业", folder_name="第一次作业", homework_type="lab_report"
        )
    ...
    def test_comment_template_recommendation(self):
        ...
        for comment in comments:
            service.record_comment_usage(teacher=self.teacher, comment_text=comment)

        # 步骤2: 获取个人评价模板
        personal_templates = service.get_personal_templates(teacher=self.teacher, limit=5)
        ...
        if len(personal_templates) >= 2:
            self.assertGreaterEqual(
                personal_templates[0].usage_count, personal_templates[1].usage_count
            )
...
class EndToEndPerformanceTest(TestCase):
    ...
    def test_large_batch_operations_performance(self):
        ...
        for file_path in file_paths:
            ...
            if cell:
                write_to_teacher_signature_cell(cell, "B", "性能测试", "教师（签字）：")
                doc.save(file_path)
        ...
        # 20个文件应该在10秒内完成
        self.assertLess(
            elapsed_time, 10.0, f"批量操作应该在10秒内完成，实际用时：{elapsed_time:.2f}秒"
        )
