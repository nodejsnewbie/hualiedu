...
from grading.models import Tenant, UserProfile, Repository, Course, Semester, Homework
...
class LabReportCommentValidationPropertyTest(HypothesisTestCase):
    ...
    def test_property_lab_report_without_comment_rejected(self, grade):
        ...
        self.assertEqual(
            ...
            200,
            f"请求应该返回200状态码，但返回 {response.status_code}"
        )
    ...
    def test_property_lab_report_with_comment_accepted(self, grade, comment):
        ...
        comment_data = comment_response.json()
        self.assertTrue(
            comment_data.get('success'),
            f"保存评价应该成功，但返回: {comment_data}"
        )
