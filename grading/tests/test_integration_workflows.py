...
from grading.models import (
    Semester, Course, Homework, Repository, Tenant, UserProfile,
    Class, Submission, CommentTemplate, GradeTypeConfig
)
...
class FilesystemRepositoryWorkflowTest(TestCase):
    ...
    def test_filesystem_repository_workflow(self):
        ...
        # 步骤5: 验证目录结构
        is_valid = self.repository_service.validate_directory_structure(
            repo_full_path
        )
...
class CommentCacheAndTemplateWorkflowTest(TestCase):
    ...
    def test_comment_deduplication(self):
        ...
        # 步骤3: 验证数据库中只有一条记录
        templates_in_db = CommentTemplate.objects.filter(
            teacher=self.user,
