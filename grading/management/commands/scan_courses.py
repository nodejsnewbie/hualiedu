...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if not current_semester:
            self.stdout.write(self.style.ERROR("没有可用的学期，请先创建学期"))
            return
        ...
        if teacher_username:
            try:
                ...
                self.stdout.write(self.style.ERROR(f"用户不存在: {teacher_username}"))
                return
        else:
            teacher = User.objects.filter(is_staff=True).first()
            if not teacher:
                self.stdout.write(self.style.ERROR("没有可用的教师用户"))
                return
        ...
        for item in os.listdir(repo_path):
            ...
            if existing:
                ...
                if existing.course_type != course_type:
                    ...
                    self.stdout.write(f"[跳过] {course_name} (已存在，类型: {course_type})")
                    skipped_count += 1
                continue

            if dry_run:
                self.stdout.write(self.style.WARNING(f"[预览-创建] {course_name} -> {course_type}"))
            else:
                ...
                self.stdout.write(self.style.SUCCESS(f"[已创建] {course_name} -> {course_type}"))
