...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        try:
            ...
            self.stdout.write(self.style.ERROR(f"课程不存在: {course_name}"))
            return
        ...
        if not os.path.exists(repo_path):
            self.stdout.write(self.style.ERROR(f"路径不存在: {repo_path}"))
            return
        ...
        for item in os.listdir(repo_path):
            ...
            # 检查是否已存在
            existing = Homework.objects.filter(course=course, folder_name=item).first()

            if existing:
                self.stdout.write(f"[跳过] {item} (已存在)")
                skipped_count += 1
                continue

            if dry_run:
                self.stdout.write(self.style.WARNING(f"[预览] {item} -> {homework_type}"))
            else:
                ...
                self.stdout.write(self.style.SUCCESS(f"[已导入] {item} -> {homework_type}"))
        ...
        if dry_run:
            ...
            self.stdout.write(
                f"运行 python manage.py import_homeworks {repo_path} {course_name} 以实际导入"
            )
        else:
            self.stdout.write(
                self.style.SUCCESS(f"成功导入了 {imported_count} 个作业，跳过 {skipped_count} 个")
            )
