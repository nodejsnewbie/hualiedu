    ...
    def handle(self, *args, **options):
        ...
        for course in courses:
            ...
            if is_lab and course.course_type == "theory":
                ...
                if dry_run:
                    self.stdout.write(
                        self.style.WARNING(f"[预览] 课程: {course.name} ({old_type} -> {new_type})")
                    )
                else:
                ...
                self.stdout.write(f"[跳过] 课程: {course.name} (类型: {course.course_type})")

        self.stdout.write("-" * 80)

        if dry_run:
            self.stdout.write(
                self.style.WARNING(f"预览模式：将更新 {updated_count} 个课程（未实际更新）")
            )
            self.stdout.write("运行 python manage.py update_course_types 以实际更新")
        else:
            self.stdout.write(self.style.SUCCESS(f"成功更新了 {updated_count} 个课程的类型"))
