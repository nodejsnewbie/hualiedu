...
from grading.models import Assignment, Course, Class
...
class AssignmentManagementService:
    ...
    def list_assignments(
        ...
        queryset = Assignment.objects.filter(
            owner=teacher,
            tenant=teacher.userprofile.tenant
        )
    ...
    def get_teacher_courses(self, teacher: User) -> QuerySet[Course]:
        ...
        """
        # 获取该教师所有作业关联的课程ID
        course_ids = Assignment.objects.filter(
            owner=teacher,
            tenant=teacher.userprofile.tenant
    ...
    def update_assignment(
        ...
        **update_fields
        ...
        Note:
            ...
            - Git密码会自动加密存储
        """
        from grading.assignment_utils import ValidationError, PathValidator, CredentialEncryption
        ...
        # 租户检查：确保租户一致
        if assignment.tenant != teacher.userprofile.tenant:
            logger.warning(
                f"Teacher {teacher.username} attempted to update assignment {assignment.id} "
        ...
        allowed_fields = {
            ...
            'git_url', 'git_branch', 'git_username', 'git_password',
            'base_path'
        }
        ...
        if assignment.storage_type == 'git':
            if 'git_url' in valid_updates:
                ...
                # 简单的URL格式验证
                if not (git_url.startswith('http://') or 
                        git_url.startswith('https://') or 
                        git_url.startswith('git@') or
        ...
        try:
            ...
            logger.error(
                f"Failed to save assignment {assignment.id}: {str(e)}",
                exc_info=True
            )
            raise ValidationError(
                f"保存作业配置失败: {str(e)}",
    ...
    def get_student_courses(self, student: User) -> QuerySet[Course]:
        ...
        # 获取学生所在的班级
        # 注意：这里假设学生通过某种方式关联到班级
        # 实际实现可能需要根据具体的数据模型调整
        # 例如：通过 StudentClass 中间表或其他关联方式
        
        # 方案1：如果学生直接关联到班级（通过 UserProfile 或其他方式）
        # 这里我们假设通过 Class 模型的某个字段或关联来获取
        
        # 方案2：通过作业提交记录反向查找学生的班级
        # 获取学生提交过作业的所有班级
        from grading.models import Submission
        
        class_ids = Submission.objects.filter(
            student=student
        ).values_list('homework__class_obj_id', flat=True).distinct()
        ...
        ).distinct().select_related('semester').order_by('-semester__start_date', 'name')
        
        logger.info(
            f"Retrieved {courses.count()} courses for student {student.username}"
        )
            ...
            if assignment.storage_type == "git":
                ...
                if assignment.git_password_encrypted:
                    try:
                        ...
                        logger.warning(f"Failed to decrypt git password: {e}")
    ...
    def upload_student_file(
        ...
        """
        from grading.assignment_utils import ValidationError, PathValidator
        from grading.services.filesystem_storage_adapter import FileSystemStorageAdapter
        
        # 只支持文件系统存储
        if assignment.storage_type != "filesystem":
            raise ValidationError(
                "Only filesystem storage supports file upload",
                "只有文件上传方式的作业支持此操作"
            )
        ...
        supported_formats = ['.docx', '.doc', '.pdf', '.txt', '.xlsx', '.xls', '.zip', '.rar', '.jpg', '.png']
        ...
        try:
            ...
            logger.error(
                f"Failed to upload file for student {student.username}: {e}",
                exc_info=True
            )
            raise ValidationError(
    ...
    def create_assignment_number_directory(
        ...
        try:
            ...
            if auto_generate_name:
                ...
                if not custom_name:
                    raise ValidationError(
                        "Custom name required when auto_generate_name=False",
                        "未提供自定义名称"
                    )
    ...
    def generate_file_storage_path(
        ...
        logger.debug(
            f"Generated file storage path for assignment {assignment.id}: {full_path}"
        )
        
        return full_path

    def validate_class_directory_isolation(
        self,
        assignment: Assignment,
        path: str
    ) -> bool:
        ...
        if not target_path.startswith(base_path):
            ...
            raise ValidationError(
                ...
                "无效的路径：不允许访问班级目录之外的文件"
            )
        
        logger.debug(
            f"Path validation passed for assignment {assignment.id}: {path}"
        )
        
        return True

    def get_class_assignment_path(
        self,
        course: Course,
        class_obj: Class
    ) -> str:
