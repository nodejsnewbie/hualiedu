  ...
  return (
    <div className="card">
      <div className="card-header">
        <h4 className="mb-0">Homework Upload</h4>
      </div>
      <div className="card-body">
        {message ? <div className="alert alert-info">{message}</div> : null}
        {loading ? <div className="alert alert-info">Loading...</div> : null}
        {!loading && homeworks.length === 0 ? (
          <div className="alert alert-secondary">No homework available.</div>
        ) : (
            ...
            {homeworks.map((homework) => (
              <div key={homework.id} className="border rounded p-3 bg-white">
                ...
                {homework.submissions && homework.submissions.length > 0 ? (
                  <div className="mt-3">
                    <div className="small text-muted">Submission history</div>
                    <ul className="list-group list-group-flush">
                      {homework.submissions.map((submission) => (
                        <li key={submission.id} className="list-group-item px-0">
                          <div className="small">
                            {submission.file_name} · version {submission.version} ·{' '}
