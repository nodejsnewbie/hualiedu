...
class GradeRegistration:
    def __init__(self):
        ...
        self.default_excel_path = Path(
            "/Users/linyuan/jobs/22g-class-java-homework/平时成绩登记表-22计算机G1班.xlsx"
        )
        # 设置默认仓库路径
        self.repo_path = Path(".")
    ...
    def get_homework_number_from_path(self, file_path: str) -> int:
        ...
        # 从目录名中提取作业次数
        match = re.search(
            r"第([一二三四五六七八九十]+)次作业|作业([一二三四五六七八九十]+)|作业(\d+)",
            dir_name,
        )
        if not match:
        ...
        try:
            ...
            with open(excel_path, "rb") as f:
                ...
                if excel_path not in self._student_cache:
                    ...
                    self._student_cache[excel_path] = self._load_student_list(
                        Path(excel_path)
                    )
    ...
    def write_grade_to_excel_by_dirname(
        self, excel_path: str, student_name: str, homework_dir_name: str, grade: str
    ) -> None:
        """
                ...
                if "老师评分：" in paragraph.text:
                    ...
                    if grade_text:
                        if grade_text in ["A", "B", "C", "D", "E"]:
                            ...
                            raise ValueError(
                                f"Invalid grade '{grade_text}' in file: {path}"
                            )
    ...
    def process_docx_files(self, repository_path: str) -> None:
        ...
        for excel_file in excel_files:
            ...
            class_name = excel_file.stem[
                index + 1 :
            ]  # 例如：23计算机1-2班 或 22计算机G1班
            ...
            logger.info(
                f"根据成绩登记表判断: {'多班级' if is_multi_class else '单班级'}"
            )

            if is_multi_class:
                self._process_multi_class(str(repo_path))
            else:
                self._process_single_class(
                    str(repo_path), str(excel_file), year, classes
                )

        logger.info("=" * 50)

    def _process_single_class(
        self, repo_path: str, excel_file: str, year: str, major: str
    ) -> None:
        """处理单班级仓库"""
        logger.info("处理单班级仓库")
        logger.info("=" * 50)
        ...
        for item in os.listdir(repo_path):
            ...
            if os.path.isdir(item_path) and re.match(
                r"第[一二三四五六七八九十]+次作业|作业[一二三四五六七八九十]+|作业\d+",
                item,
            ):
    ...
    def _process_multi_class(self, repo_path: str) -> None:
        ...
        for excel_file in excel_files:
            ...
            file_name = os.path.basename(excel_file)
            match = re.match(
                r"平时成绩登记表-(\d{2})计算机(\d+)-(\d+)班\.xlsx", file_name
            )
            if not match:
        ...
        for docx_file in docx_files:
            try:
                ...
                self.write_grade_to_excel(
                    excel_file, student_name, homework_dir_name, grade
                )
            except Exception as e:
