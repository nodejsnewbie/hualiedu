...
def diagnose(course_name, homework_folder):
    ...
    try:
        ...
        print()
        print("解决方案：")
        print(f"  1. 使用导入命令: conda run -n py313 python manage.py import_homeworks <仓库路径> {course_name}")
    ...
    for repo in repositories:
        ...
        for path in possible_paths:
            if os.path.exists(path) and os.path.isdir(path):
                ...
                word_files = [f for f in files if f.endswith(('.docx', '.doc')) and not f.startswith('~$')]
