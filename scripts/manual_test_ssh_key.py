...
-----END OPENSSH PRIVATE KEY-----"""
    ...
    ssh_key_file = SimpleUploadedFile(
        "id_rsa", test_ssh_key.encode("utf-8"), content_type="text/plain"
    )
    ...
    invalid_file = SimpleUploadedFile(
        "invalid.txt", b"This is not a valid SSH key", content_type="text/plain"
    )

    response = client.post(auth_config_url, {"ssh_private_key_file": invalid_file})

    print(f"无效文件上传状态码: {response.status_code}")

    # 测试清除SSH私钥
    print("\n🗑️ 测试清除SSH私钥...")
    response = client.post(auth_config_url, {"clear_ssh_key": "1"})
